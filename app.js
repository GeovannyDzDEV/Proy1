...
const contenedorProductos = document.getElementById('contenedor-productos') //Nos migramos a las constante contenedorProductos del declarado en el index
...
//localstoragepara que no se pierdan los datos al recargar la pagina
...
//SEXTO PASO  Boton vaciar carrito
...
stockProductos.forEach((producto) => {
    ...
    div.classList.add('producto')//estilo del producto e inyectamos html
    div.innerHTML = `                      
    ...
    `
    contenedorProductos.appendChild(div) //con esto ya se muestra el stock
...
// 5 - QUINTO PASO 
//ahora realizamosla función de eliminar del carrito
const eliminarDelCarrito = (prodId) => {
    const item = carrito.find((prod) => prod.id === prodId) //eliminamos alosproductos del carrito de los productos

    const indice = carrito.indexOf(item) //Busca el elemento q yo le pase y nos devuelve su indice.

    carrito.splice(indice, 1) //Le pasamos el indice de mi elemento ITEM y borramos 1 en cuanto a cantidad
    // un elemento 
    actualizarCarrito() //LLAMAMOS A LA FUNCION QUE CREAMOS EN EL TERCER PASO. CADA VEZ Q SE 
    //MODIFICA EL CARRITO
    console.log(carrito)
